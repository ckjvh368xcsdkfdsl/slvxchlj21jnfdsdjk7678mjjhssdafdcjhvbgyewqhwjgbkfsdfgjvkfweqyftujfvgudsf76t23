		...
		function UILibrary:Start(Parameters)
			...
			do
				local FakeBackGround = UIUtilities:Create("Frame", {
					...
					BorderColor3 = UIStyle.UIcolors.Accent,
...
local radius = 10
	...
	do
		...
		UILibrary:CreateSlider({Name = "Maximum Lock-On Time", Tab = "Legit", Section = "Aim Assist", MinimumNumber = 0, MaximumNumber = 8500, Suffix = "ms", MaximumText = "Inf", Tooltip = "Controls how long the aim assist will aim at a target before re-considering what to aim at"})
		...
		UILibrary:CreateSlider({Name = "Back Tracking Time", Tab = "Rage", Section = "Tracking", Suffix = "ms", MinimumNumber = 0, MaximumNumber = 8500, Tooltip = "Controls how far back in time the enemy positions are saved."})
		...
		UILibrary:CreateSlider({Name = "Extrapolation Tuning", Tab = "Rage", Section = "Tracking", Suffix = "ms", MinimumNumber = 0, MaximumNumber = 8500, Tooltip = "Controls how much extra time ahead an enemy is extrapolated."})
	...
	game.Players.PlayerRemoving:Connect(function()
		...
		UILibrary:CreateTextBox({Name = "Change To", Tab = "Visuals", Section = "Player", Default = "BloxSense User"})
...
local function ApplyTexture(ins, id)
...
local function EnableMinecraft()
    if texturesApplied then return end
...
local function DisableMinecraft()
    if not texturesApplied then return end
...
task.spawn(function()
    while task.wait(0.1) do
        if Menu["Misc"]["Exploits"]["Minecraft Textures"]["Toggle"]["Enabled"] == true then
            EnableMinecraft()
        else
            DisableMinecraft()
        end
		...
		UILibrary:CreateButton({Name = "Aurora Watermark", Tab = "Settings", Section = "Menu Settings"})
			...
			rs.RenderStepped:Connect(function()
				if Menu["Settings"]["Menu Settings"]["Aurora Watermark"]["Toggle"]["Enabled"] then
					...
					textLabel.Text =
						"  <font color=\"#FF2525\">bloxsense.pub</font> <font color=\"#3f3f3f\" size=\"9\">v0.1.3</font> " ..
						GetFramerate() .. " fps 〡" ..
		...
		UILibrary:CreateTextBox({Name = "Custom Menu Name Text", Tab = "Settings", Section = "Menu Settings", Default = "bloxsense.pub"})
...
for i,v in next, character:GetChildren() do
    if v:IsA("BasePart") and v.Name ~= "HumanoidRootPart" and v.Name ~= "Gun" and v.Name ~= "BackC4" then
        ...
        if weld then
            ...
            data.step = runService.Stepped:Connect(function()
                local section = this and this.theirpInfo and this.theirpInfo.enemy and "Enemy ESP" or "Team ESP"
                if Menu["ESP"][section]["Enabled"]["Toggle"]["Enabled"] 
                and Menu["ESP"][section]["Skeleton"]["Toggle"]["Enabled"] then
                    ...
                    if v0 and v1 then
                        data.line.From = newVector2(p0.x, p0.y)
                        data.line.To   = newVector2(p1.x, p1.y)
...
runService.Heartbeat:Connect(function()
    if scopelb then
		...
		function visuals.updateGUI(upTime, deltaTime) -- pasted from vadersense
	...
	do --ANCHOR Ragebot
		...
		function ragebot.setweapons()
			...
			if replicatedStorage:FindFirstChild("Weapons") then
				...
				client.secondaryskin = "bloxsense"
...
misc.updateweaponstats = function()
    ...
    if Menu["Misc"]["Weapon Modifications"]["Infinite Ammo"]["Toggle"]["Enabled"] then
		game:GetService("Players").LocalPlayer.PlayerGui.GUI.AmmoGUI.AmmoClip.Text = 'blox'
        game:GetService("Players").LocalPlayer.PlayerGui.GUI.AmmoGUI.AmmoReserve.Text = 'sense'
		...
		legitbot.legitbotloop = runService.Stepped:Connect(function(delta)
			...
			if not client.fgun or client.fgun == "none" or not localPlayer.Character or (localPlayer.Character and not localPlayer.Character:FindFirstChild("Humanoid")) then -- hmm i wonder where ive seen this b4
				...
				if legitbot.triggerbottarget.magnet.player then
					...
					local canLockOn = Menu["Legit"]["Aim Assist"]["Maximum Lock-On Time"]["Value"] == 8500 or legitbot.aimingTime < Menu["Legit"]["Aim Assist"]["Maximum Lock-On Time"]["Value"]
